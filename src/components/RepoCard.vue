<template>
  <div class="cardBox" v-if="!loading">
    <div class="boxHeader">
      <svg class="repoIcon" viewBox="0 0 16 16" width="16" height="16" aria-hidden="true">
        <path fill-rule="evenodd" d="M2 2.5A2.5 2.5 0 014.5 0h8.75a.75.75 0 01.75.75v12.5a.75.75 0 01-.75.75h-2.5a.75.75 0 110-1.5h1.75v-2h-8a1 1 0 00-.714 1.7.75.75 0 01-1.072 1.05A2.495 2.495 0 012 11.5v-9zm10.5-1V9h-8c-.356 0-.694.074-1 .208V2.5a1 1 0 011-1h8zM5 12.25v3.25a.25.25 0 00.4.2l1.45-1.087a.25.25 0 01.3 0L8.6 15.7a.25.25 0 00.4-.2v-3.25a.25.25 0 00-.25-.25h-3.5a.25.25 0 00-.25.25z"></path>
      </svg>
      <span class="name">
        <a style="text-decoration: none; color: inherit;" :href="url">{{repoName}}</a>
      </span>
    </div>
    ...
    <div class="infoBox">
      <div class="langBox" v-if="lang">
        <span class="language" :style="{backgroundColor: this.langColor[lang].color}"></span>
        <span> {{ lang }}</span>
      </div>
      <div class="starsBox" v-if="stars > 0">
        <svg aria-label="stars" viewBox="0 0 16 16" width="16" height="16" role="img">
      ...
      <div class="forksBox" v-if="forks > 0">
        <svg aria-label="fork" viewBox="0 0 16 16" width="16" height="16" role="img">
  ...
  created() {
    if(this.repo !== undefined){
      ...
      axios.get("https://api.github.com/repos/" + this.repo)
        .then(r => {
          console.log(r.data)
          vm.url = r.data.html_url;
          vm.repoName = r.data.name;
          vm.isFork = r.data.fork;
          if(r.data.fork){
            vm.forkPath = r.data.source.url;
            vm.forkName = r.data.source.full_name;
          }
          vm.description = r.data.description;
          vm.lang = r.data.language;
          vm.stars = r.data.stargazers_count;
          vm.forks = r.data.forks_count;
          vm.loading = false;
        })
    } else {
      this.loading = false;
      this.url = this.OfflineData.url;
      this.repoName = this.OfflineData.repoName;
      this.isFork = this.OfflineData.isFork;
      if(this.isFork){
        this.forkPath = this.OfflineData.forkPath;
        this.forkName = this.OfflineData.forkName;
      }
      this.description = this.OfflineData.description;
      this.lang = this.OfflineData.lang;
      this.stars = this.OfflineData.stars;
      this.forks = this.OfflineData.forks;
    }
  },
  props: {
    repo: String,
